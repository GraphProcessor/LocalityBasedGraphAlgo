...
mwIndex gsqexpmseed(sparse_row *graph, sparse_vec &set, sparse_vec &y, const double t, const double eps,
...
int hyper_cluster_heat_kernel_multiple(sparse_row *G, const vector<mwIndex> &seed_set, double t, double eps,
                                       sparse_vec &p, sparse_vec &r, Queue &q, vector<mwIndex> &cluster,
    ...
    auto nsteps = gsqexpmseed(G, r, p, t, eps, static_cast<size_t >(ceil(pow(G->n_, 1.5))), q);
...
void copy_array_to_index_vector(const mxArray *v, vector<mwIndex> &vec) {
    size_t n_ = mxGetNumberOfElements(v);
    double *p = mxGetPr(v);
